...
class MovieLens(BasicDataset):
    ...
    def getSparseGraph(self):
        if self.Graph is None:
            ...
            # Compute the sum of non-zero items in each row of A_csr
            A_row_sum = np.array(A_csr.sum(axis=1)).squeeze()

            # Count the number of non-zero items in each row of A_csr
            A_row_nnz = np.array(A_csr.getnnz(axis=1)).squeeze()

            # Compute the mean of non-zero items in each row of A
            rowmean = np.zeros_like(A_row_sum)
            rowmean[A_row_sum!=0] = A_row_sum[A_row_sum!=0] / A_row_nnz[A_row_sum!=0]
            rowmean[A_row_sum == 0] = A_row_sum[A_row_sum == 0]
            #print(rowmean)

            #norm_adj=np.zeros_like(adj_mat, dtype=np.float64)
            #norm_adj[adj_mat != 0] = (1 / ((adj_mat[adj_mat != 0] - rowmean[:, np.newaxis])**2))
            #adj_mat = sp.dok_matrix((self.n_users + self.m_items, self.n_users + self.m_items), dtype=np.float32)
            for i, j in zip(*adj_mat.nonzero()):
                adj_mat[i, j] = 1 / ((adj_mat[i, j] - rowmean[i])**2 + 0.1) ** 0.5
            A_t = adj_mat.transpose()
            for i, j in zip(*A_t.nonzero()):
                A_t[i, j] = A_t[i, j] / (((A_t[i, j] - rowmean[i])**2 + 0.1) ** 0.5)
            #print(type(adj_mat))
            #print(adj_mat)
            adj_mat = A_t.transpose()
            '''d_inv = np.power(rowmean, -0.5).flatten()
            d_inv[np.isinf(d_inv)] = 0.
            d_mat = sp.diags(d_inv)
            
            norm_adj = d_mat.dot(adj_mat)
            norm_adj = norm_adj.dot(d_mat)'''
            norm_adj = sp.dok_matrix((self.n_users + self.m_items, self.n_users + self.m_items), dtype=np.float32)
            norm_adj[adj_mat != 0]=adj_mat[adj_mat != 0]
            norm_adj=norm_adj.tocsr()
            #print(norm_adj)
            end = time()
            print(f"costing {end-s}s, saved norm_mat...")
